...
def obfuscate_code(code):
    ...
    while i < ln:
        ...
        # Skip/keep string literals intact (we don't transform inside them here)
        if ch == '"':
            j = i + 1
            while j < ln:
                if code_after_funcdefs[j] == '\\':
                    j += 2
                    continue
                if code_after_funcdefs[j] == '"':
                    j += 1
                    break
                j += 1
            out.append(code_after_funcdefs[i:j])
            i = j
            continue
        if ch == '`':
            j = code_after_funcdefs.find('`', i+1)
            if j == -1:
                out.append(code_after_funcdefs[i:])
                break
            out.append(code_after_funcdefs[i:j+1])
            i = j+1
            continue
        if ch == "'":
            j = i + 1
            ...
            i = j
            continue
        if code_after_funcdefs.startswith('//', i):
            j = code_after_funcdefs.find('\n', i+2)
            if j == -1:
                out.append(code_after_funcdefs[i:])
                break
            out.append(code_after_funcdefs[i:j+1])
            ...
            continue
        if code_after_funcdefs.startswith('/*', i):
            j = code_after_funcdefs.find('*/', i+2)
            if j == -1:
                out.append(code_after_funcdefs[i:])
                break
            out.append(code_after_funcdefs[i:j+2])
            i = j+2
            continue
        if IDENT_START.match(ch):
            ...
            k = start - 1
            while k >= 0 and code_after_funcdefs[k].isspace():
                k -= 1
            prev_char = code_after_funcdefs[k] if k >= 0 else ''
            replaced = ident
